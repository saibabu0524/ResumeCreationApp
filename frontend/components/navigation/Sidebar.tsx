...
import { LayoutDashboard, Wand2, Target, LogOut, FileText, X } from "lucide-react";
...
interface SidebarProps {
    /** Called when the user taps the close button on mobile */
    onClose?: () => void;
}
...
export function Sidebar({ onClose }: SidebarProps) {
    ...
    return (
        <aside className="flex h-full w-72 flex-col glass-strong border-r border-border/60 lg:w-64 lg:h-screen">
            {/* Logo + mobile close */}
            <div className="flex items-center gap-2.5 px-5 py-5 border-b border-border/40">
                <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-primary/15 ring-1 ring-primary/30 shrink-0">
                    <FileText className="h-4 w-4 text-primary" />
                </div>
                <span className="flex-1 text-lg font-bold gradient-text">ResumeTailor</span>
            ...
            <div className="border-t border-border/40 px-3 py-4 space-y-2">
                ...
                <button
                    ...
                    className="flex w-full items-center gap-3 rounded-lg px-3 py-3 text-sm font-medium text-muted-foreground
